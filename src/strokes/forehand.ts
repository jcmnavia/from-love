...
/**
 * Forehand racket arm keyed frame by frame from Sinner's forehand (front and side practice footage, contact = 0);
 * the capture's hand path already matches his within ~10 cm, so the hand is left to the clip outside the loop.
 */
export const FOREHAND_ARM_KEYS: ArmKey[] = [
  ...
  // recovery: the racket comes back up in front with the left hand catching it (the clip loops at +0.557)
  { t: 0.5, racket: { dir: [-0.1, 0.9, 0.42] } },
